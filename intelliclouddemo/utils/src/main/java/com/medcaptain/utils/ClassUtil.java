...
public final class ClassUtil {
    ...
    public static String getBriefClassName(Class clazz) {
        if (clazz == null) {
            return null;
        } else {
            String className = clazz.getName();
            return getBriefClassName(className);
        }
    }

    public static String getBriefClassName(String className) {
        StringBuffer briefClassNameBuffer = new StringBuffer();
        if (StringUtils.isNotEmpty(className)) {
            String[] classNames = className.split("\\.");
            for (int i = 0; i < classNames.length; i++) {
                if (i < classNames.length - 2) {
                    briefClassNameBuffer.append(classNames[i].substring(0, 1));
                    briefClassNameBuffer.append(".");
                } else if (i == classNames.length - 2) {
                    briefClassNameBuffer.append(classNames[i]);
                    briefClassNameBuffer.append(".");
                } else {
                    briefClassNameBuffer.append(classNames[i]);
                }
        ...
        return briefClassNameBuffer.toString();
    }
}
