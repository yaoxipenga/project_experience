...
public class DateUtil {
     ...
     *
     * @param startDate 开始日期
     * @param endDate   结算日期  （包含）
     * @return 日期列表
     */
    public static List<Date> getEveryYear(Date startDate, Date endDate) {
        if (startDate == null || endDate == null) {
            return null;
        }
        // 格式化日期(yy-MM-dd)
     ...
     * @return 指定月份集合
     */
    public static List<String> getDateMonthList(int year, int month, int len) {
        System.out.println("year:" + year + "month" + month + "len" + len);
        ...
        if (len == 0) {
            returnList.add(getYearMonthString(year, month));
        } else {
    ...
    private static String getYearMonthString(int year, int month) {
