...
for ((i=0;i<${SERVICE_NUM};i++))
{
  ...
  echo "    - ${LOG_DIR}:${LOG_DIR}:rw" >> docker-compose.yml
  echo "    - /tmp" >> docker-compose.yml
  echo "    command: java -Djava.security.egd=file:/dev/./urandom -jar -Xms1024m -Xmx2048m ${JAR_DIR}/${SERVICE_NAME_ARRAY[i]}-${SERVER_VERSION} --spring.profiles.active=online --eureka.instance.ip-address=${IP_ADDRESS} --spring.cloud.config.uri=http://192.168.4.11:8777" >> docker-compose.yml
  echo -e "\n" >> docker-compose.yml
}
fi
